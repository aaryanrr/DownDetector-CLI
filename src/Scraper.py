        ...
        try:
            ...
            if not status_text:
                ...
                def is_valid_status_element(elem):
                    ...
                    if any(
                        "status" in str(c).lower() or "entry-title" in str(c).lower()
                        ...
                        if (
                            text
                            and len(text) > MIN_STATUS_TEXT_LENGTH
                            and text.lower() not in ["status", "info", "information"]
                        ):
                            return True
                    return False
                ...
                status_elem = next(
                    (
                        ...
                        if is_valid_status_element(elem)
                    ),
                    None,
                )
                if status_elem:
            ...
            raise Errors.NetworkError(
                "Could not fetch the page. Please check your internet connection and try again."
            )
...
def check_connection():
    try:
        response = requests.get(
            "https://downdetector.com", headers={"User-Agent": USER_AGENT}, timeout=10
        )
        response.raise_for_status()
        print("All Good 👍")
